...
| **worktree** | `/worktree [issue\|slug\|list\|remove]` | Full git-worktree lifecycle: start one on a task-named branch (based on `origin/<default>`), bootstrap the gitignored files a fresh checkout lacks (`.claude/` symlinks, env files, dependencies), and switch the session into it via `EnterWorktree`. Teardown runs a safety sweep — uncommitted work, unpushed commits, PR state — and never discards work without showing exactly what would be lost |
